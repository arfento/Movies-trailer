...
class MoviesViewModel(
    ...
    private val movieResult: LiveData<MovieResult> = Transformations.map(
        movieTypeSelected
    ) { movieType ->
    ...
    var networkErrors: LiveData<String> =
        Transformations.switchMap(movieResult) { it.networkErrors }
    var loadingSpinner: LiveData<Boolean> =
        Transformations.switchMap(movieResult) { it.loadingData }

    val searchedMovies: LiveData<PagedList<Movie>> =
        Transformations.switchMap(movieSearchResult) { it.data }
    val searchNetworkErrors: LiveData<String> =
        Transformations.switchMap(movieSearchResult) { it.networkErrors }
